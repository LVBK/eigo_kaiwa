class UserLogsController < ApplicationController
  ...
  def index
    ...
    @sentence_learnings = current_user.user_logs.all
    @groups_for_chart = @sentence_learnings.to_a.group_by{|x| x.created_at.beginning_of_day}.map{|group| [group[0].strftime("%F"), group[1].map(&:sentence_learning).sum]}
  end
  ...
  private
    ...
    def user_log_params
      params.require(:user_log).permit(:sentence_learning, :created_at)
    end
end
